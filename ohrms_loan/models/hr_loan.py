...
class HrEmployeeContract(models.Model):
    ...
    new_wage = fields.Float(string="Total Salary", required=False, compute="_compute_new_wage")

    def _compute_new_wage(self):
        for this in self:
            this.new_wage = this.wage + this.house_allow + this.trans_allow + this.phone_allow + this.food_allow + this.other_allow + this.internal_mandate_allowances + this.external_mandate_allowances + this.internal_travel_allowances + this.abroad_travel_allowances + this.other_travel_allowances + this.hour_travel_allowances - this.ded_gosi - this.ded_employee_gosi - this.ded_company_gosi - this.ded_bank_fees - this.ded_other
...
class HrLoan(models.Model):
    ...
    loan_amount = fields.Float(string="Loan Amount", required=False, store=True)
    ...
    loan_duration = fields.Selection(string="Loan Duration",
                                     selection=[('one_month', 'One Month'), ('two_month', 'Two Month'), ],
                                     required=False, )

    new_wage = fields.Float(string="",  required=False, )

    @api.multi
    @api.onchange('loan_duration', 'employee_id')
    def onchange_loan_duration(self):
        for rec in self:
            print("ahmed saber")
            contract_id = rec.employee_id.contract_id
            amount = rec.employee_id.contract_id.new_wage
            if rec.loan_duration == 'one_month':
                rec.loan_amount = amount
            elif rec.loan_duration == 'two_month':
                rec.loan_amount = amount * 2
            else:
                return
        else:
            return
