...
class attendance_sheet(models.Model):
    ...
    @api.multi
    def unlink(self):
        if any(self.filtered(lambda att: att.state not in ('draft', 'confirm'))):
            raise UserError(_('You cannot delete an attendance sheet which is not draft or confirmed!'))
        return super(attendance_sheet, self).unlink()

    @api.constrains('date_from', 'date_to')
    @api.multi
    def check_date(self):
        for sheet in self:
            emp_sheets = self.env['attendance.sheet'].search([('employee_id', '=', sheet.employee_id.id)])
            for emp_sheet in emp_sheets:
                ...
                if max(sheet.date_from, emp_sheet.date_from) < min(sheet.date_to, emp_sheet.date_to):
                    raise UserError(_('You Have Already Attendance Sheet For That Period  Please pick another date !'))
...
class attendance_sheet_line(models.Model):
    ...
    att_sheet_id = fields.Many2one(comodel_name='attendance.sheet',ondelete="cascade", string='Attendance Sheet',readonly=True)
