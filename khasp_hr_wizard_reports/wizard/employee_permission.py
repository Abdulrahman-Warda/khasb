...
class khasp_employee_permission_wizard(models.TransientModel):
    _name = 'employee.permission.wizard'
    ...
    def print_report(self):
        data = {
            'model': 'employee.permission.wizard',
            'form': self.read(['employee_id','type','department_id'])[0]
        }
        print('data',self.read(['employee_id','type','department_id']))
        return self.env.ref('khasp_hr_wizard_reports.employee_permission_report_tag').report_action(self, data=data, config=False)
