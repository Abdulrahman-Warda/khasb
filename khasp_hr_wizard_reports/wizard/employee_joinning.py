...
class khasp_employee_joining_wizard(models.TransientModel):
    _name = 'employee.joining.wizard'
    ...
    def print_report(self):
        data = {
            'model': 'employee.joining.wizard',
            'form': self.read(['employee_id','type','department_id'])[0]
        }
        print('data',self.read(['employee_id','type','department_id']))
        return self.env.ref('khasp_hr_wizard_reports.employee_joining_report_tag').report_action(self, data=data, config=False)
