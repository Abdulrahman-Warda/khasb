...
class khasp_contract_date_wizard(models.TransientModel):
    _name = 'contract.date.wizard'
    ...
    def print_report(self):
        data = {
            'model': 'contract.date.wizard',
            'form': self.read(['employee_id','type','department_id'])[0]
        }
        print('data',self.read(['employee_id','type','department_id']))
        return self.env.ref('khasp_hr_wizard_reports.contract_date_report_tag').report_action(self, data=data, config=False)
