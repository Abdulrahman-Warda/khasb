...
class MrpProductionInherit(models.Model):
	...
	def create_custom_lot_no(self,wo):
		...
		lot_serial_no = self.env['stock.production.lot'].create({'name' : lot_no,'product_id':self.product_id.id,'use_next_on_work_order_id' : wo.id})
	...
	def _workorders_create(self, bom, bom_data):
		...
		if self.product_id.tracking == 'serial' :
			...
			res[0].final_lot_id = lot_id_list[0]
