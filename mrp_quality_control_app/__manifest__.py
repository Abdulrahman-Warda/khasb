# -*- coding: utf-8 -*-
{
    'name': 'Quality Control (QC) Management for Manufacturing',
    "author": "Edge Technologies",
    'version': '12.0.1.0',
    'live_test_url': "https://youtu.be/ldkJKRjOIeQ",
    "images":['static/description/main_screenshot.png'],
    'summary': " Quality Control (QC) management for Manufacturing.",
    'description': """ This app help to user for Quality Control like Quality Check and Quality Alert with
                        Pass-Fail Type, Measure Type, Picture Type and Text Type for Manufacturing and Warehouse Operations.
                        Also Generate Quality Check and Alert Report.

This app help to user for Quality Control like Quality Check and Quality Alert for Warehouse. Also Generate Quality Check and Alert Report.

Quality Control Inspections
Internal Validation and Verification
QC features of warehouse management
QC warehouse management
QA warehouse management
WMS QUALITY ASSURANCE
Quality control
WMS Quality assurance
QUALITY CONTROL WMS
Inventory Control and Quality Control
Inventory Quality Control
Quality Control Inventory
Quality Control Management
Inventory control
warehouse quality control
 Inventory Management
 Stock Control and Inventory Management
 warehouse quality control
 Warehouse Performance Management
 Improve Quality and Productivity in Warehouse Operations
 Warehouse Operations - Quality Inspection
 Quality Inspection
 Quality Systems Warehouse Management
 Quality Assurance
 quality management
 Quality Management in Warehouse Management
 stock control
 inventory cost management
 multiple location support
 Inventory control systems
Purchase Product Quality Instpection
Quality alerts and Quality Inspections
incoming Product Quality Instpection
inventory inspection
shipment inspection
Pre-shipment inspection
shipment inspection
Pre Shipment Inspection

quality control inspections
product quality control inspections
wms quality control inspections
warehouse quality control inspections
Quality Control and Preshipment Inspection
Warehouse quality audit checklist
Quality Management
purchase quality control management
picking quality control management
delivery quality control management
incoming shipment quanlity control management
quality control team
quality control alerts
quality control points
purchase quality Inspection management
picking quality Inspection management
delivery quality Inspection management
incoming shipment quality Inspection management
Manufacturing quality Inspection team
Manufacturing quality Inspection alerts
Manufacturing quality Inspection points
MRP Quality control points
MRO management
MRP quality alerts
MRP quality Team
Manufacturing Quality control points
Manufacturing Quality Team management
Manufacturing quality alerts

warehouse QC control qc Control team purchase qc management warehouse qc management picking qc management
This app help to user for Quality Control like Quality Check and Quality Alert for manufacturing. Also Generate Quality Check and Alert Report.

MRP Quality Control Inspections
MRP Internal Validation and Verification
QC features of MRP management
QC MRP management
QA MRP management
MRP QUALITY ASSURANCE
Quality control MRP
MRP Quality assurance
QUALITY CONTROL MRP
MRP Control and Quality Control
MRP Quality Control
Quality Control MRP
Quality Control Management FOR MRP
MRP control for quality Inspection
MRP quality control INSTPECTION
 MRP Quality Inspection  Management
 MRP Control for Quality and MRP Inspection Management
 MRP quality control
 MRP Performance Management
 Improve Quality and Productivity in MRP Operations
 MRP Operations - Quality Inspection
 MRP Quality Inspection for MRP
 MRP Quality Systems MRP Management
 MRP Quality Assurance for MRP
 MRP quality management for MRP
 MRP Quality Management in MRP Management
 MRP control for Quality Inspection
 MRP cost management
 MRP control systems
MRP Product Quality Instpection
MRP Quality alerts and Quality Inspections
MRP Product Quality Instpection
MRP product quality control inspections for MRP
MRP quality audit checklistt
MRP quality control management
MRP quality Inspection management
MRP QC control qc Control team MRP qc management

Manufacturing Quality Control Inspections
Manufacturing Internal Validation and Verification
QC features of Manufacturing management
QC Manufacturing management
QA Manufacturing management
Manufacturing QUALITY ASSURANCE
Quality control Manufacturing
Manufacturing Quality assurance
QUALITY CONTROL Manufacturing
Manufacturing Control and Quality Control
Quality Control Manufacturing
Manufacturing Quality Control Management FOR Manufacturing
Manufacturing control for quality Inspection
Manufacturing quality control INSTPECTION
 Manufacturing Quality Inspection  Management
 Manufacturing Control for Quality and Manufacturing Inspection Management
 Manufacturing quality control
 Manufacturing Performance Management
 Improve Quality and Productivity in Manufacturing Operations
 Manufacturing Operations - Quality Inspection
 Manufacturing Quality Inspection for Manufacturing
 Manufacturing Quality Systems Manufacturing Management
 Manufacturing Quality Assurance for Manufacturing
 Manufacturing quality management for Manufacturing
 Manufacturing Quality Management in Manufacturing Management
 Manufacturing control for Quality Inspection
 Manufacturing cost management
 Manufacturing control systems
Manufacturing Product Quality Instpection
Manufacturing Quality alerts and Quality Inspections
Manufacturing Product Quality Instpection
Manufacturing product quality control inspections for Manufacturing
Manufacturing quality audit checklistt
Manufacturing quality control management
Manufacturing quality Inspection management
Manufacturing QC control qc Control team Manufacturing qc management QC control for Manufacturing QC control for MRP



    """,
    "license" : "OPL-1",
    'depends': ['base','sale_management','stock','purchase','account','mrp','warehouse_quality_control_app'],
    'data': [
            'views/check_wizard.xml',
            'views/mrp_menu.xml',
            'views/inherit_quality_point.xml',
            'views/workorder.xml',
            'views/views.xml'
            ],
    'installable': True,
    'auto_install': False,
    'price': 50,
    'currency': "EUR",
    'category': 'Manufacturing',

}
