...
class SprogroupPurchaseRequest(models.Model):
    ...
    po_type = fields.Selection(string="Purchase Type",
                               selection=[('internal', 'Internal Products'), ('private', 'Manufacturing Products')],
                               required=False)
    ...
    product_category_id = fields.Many2one('product.category', domain=[('internal_product', '=', True)],
                                          string='Product Category')
    product_id_private = fields.Many2one('product.category', domain=[('private_product', '=', True)],
                                         string='Product Category')
    product_id = fields.Many2one(
        'product.product', 'Product',
        domain=[('purchase_ok', '=', True)],
        track_visibility='onchange')
    purchase_ref = fields.Many2one(comodel_name="purchase.order", string="Purchase ID", required=False, )

    @api.onchange('product_category_id', 'product_id_private')
    def _onchange_product_category_id(self):
        res = {}
        if self.product_category_id:
            res['domain'] = {'product_id': [('categ_id', '=', self.product_category_id.id)]}
        elif self.product_id_private:
            res['domain'] = {'product_id': [('categ_id', '=', self.product_id_private.id)]}
        else:
    ...
    #
    #     if (self.state == 'leader_approved' and current_user.has_group('sprogroup_purchase_request_khasb.group_sprogroup_purchase_request_manager')):
    ...
    def _compute_is_editable(self):
        for rec in self:
            if rec.state in ('to_approve', 'rejected', 'done'):
                rec.is_editable = False
            else:
                rec.is_editable = True
