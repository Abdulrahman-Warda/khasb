...
class MrpBom(models.Model):
    ...
    def create(self, vals):
        ...
        if res.routing_id and config.process_costing == 'workcenter':
            for operation in res.routing_id.operation_ids:
                value = {
                    ...
                    # 'planned_hours' : operation.time_cycle_manual_hour,
                }
            ...
            for operation in res.routing_id.operation_ids:
                value = {
                    ...
                    # 'cost_kg' : operation.workcenter_id.overhead_costs_kg or False,
...
class MrpProduction(models.Model):
    ...
    def create(self, vals):
        ...
        for labour in res.bom_id.bom_labour_cost_ids:
            vals = {'operation_id': labour.operation_id.id,
                    ...
                    'debit_account_id': labour.debit_account_id.id,
                    'credit_account_id': labour.credit_account_id.id,
        ...
        for material in res.move_raw_ids:
            ...
            vals = {
                ...
                # 'debit_account_id': material.debit_account_id.id,
                # 'credit_account_id': material.credit_account_id.id,
                'cost': material.product_id.standard_price,
    ...
    @api.one
    @api.depends('pro_material_cost_ids','pro_labour_cost_ids','pro_overhead_cost_ids')
    def _compute_total_cost(self):
        material_total = 0.0
    ...
    # Costing Tab
    total_material_cost = fields.Float(compute='_compute_total_cost', string="Total Material Cost", default=0.0,store=True)
    total_labour_cost = fields.Float(compute='_compute_total_cost', string="Total Labour Cost", default=0.0,store=True)
    total_overhead_cost = fields.Float(compute='_compute_total_cost', string="Total Overhead Cost", default=0.0,store=True)
    total_all_cost = fields.Float(compute='_compute_total_all_cost', string="Total Cost", default=0.0,store=True)
    ...
    total_actual_overhead_cost = fields.Float(compute='_compute_total_cost', string="Total Actual Overhead Cost",
                                              default=0.0,store=True)
    total_actual_all_cost = fields.Float(compute='_compute_total_all_cost', string="Total Actual Cost", default=0.0,store=True)
...
class ChangeProductionQty(models.TransientModel):
    ...
    def change_prod_qty(self):
        ...
        for wizard in self:
            ...
            finished_moves_modification = self._update_product_to_produce(production,
            ...
            for wo in production.workorder_ids:
                ...
                if quantity > 0 and wo.move_raw_ids.filtered(
                        lambda x: x.product_id.tracking != 'none') and not wo.active_move_line_ids:
